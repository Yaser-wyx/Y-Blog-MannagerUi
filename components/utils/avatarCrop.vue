  ...
  <div>
    ...
    >
      <v-card flat style="position: relative;">
        <v-progress-linear class="loading ma-0" height="3" color="blue" v-if="loading"
                           :indeterminate="true"></v-progress-linear>
        <v-btn icon flat color="red" small fab class="close" @click="close">
          <v-icon color="red" size="33">iconfont blog-close</v-icon>
        </v-btn>
            ...
            <v-layout wrap align-end row>
              ...
              <v-flex md6 class="pl-5">
                ...
                <v-btn flat round color="red" @click="flipXY('Y')" icon
                       style="margin: 0!important;">
                  <v-icon :size="33" color="red">iconfont blog-flip-y</v-icon>
                </v-btn>
              </v-flex>
              ...
              <v-flex md12 class="text-md-right">
                <v-btn round color="#18ADED" style="margin: 0!important;" depressed dark
                       @click="choose">
                  选取图片
                </v-btn>
              </v-flex>
          ...
          <v-flex md6 class="pl-4 ml-2">
            <v-layout wrap row>
              ...
              <v-flex md12 class="mt-4 text-md-right">
                <v-btn large color="#58D68D" depressed dark class="title" round
    ...
    <v-avatar
      ...
      @click="dialog=true"
    >
      <img :src="nowImage" v-if="nowImage!==null">
      <v-icon :size="size" v-else>iconfont blog-user-circle</v-icon>
    </v-avatar>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      uploadCroppedImage() {
        let avatarFile = dataURLtoFile(this.image, 'image.jpeg')
        this.beforeUpload(avatarFile).then(() => {
          this.apis.serverApi.uploadAvatar(avatarFile).then(res => {
            console.log(res)
            this.handleSuccess(res)
            this.$emit("setAvatarValid", true)
            this.close()
          }).catch(err => {
            this.handleError()
          })
      ...
      fileSizeExceed(){
        this.$message.warning("文件不要超过1MB，请重新选择！")
      },
